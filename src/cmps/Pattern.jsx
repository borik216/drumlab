...
export default function Pattern({ index }) {
  ...
  return (
    <PatternContextProvider index={index}>
      {isEditMode &&<PatternToolbar patternIndex={index} />}
      <div className="flex max-w-3xl mx-auto mb-6">
        <InstrumentRack />
...
function PatternToolbar({ patternIndex }) {
  ...
  const buttonClass = 'flex justify-center items-center flex-1 bg-gray-200 hover:bg-gray-300'
  return (
    <div className="w-2/6 flex ml-auto divide-x-2 divide-slate-300 border border-slate-300">
      <button className={buttonClass} onClick={generateRandomPattern}><RandomIcon /></button>
      <button className={buttonClass} onClick={resetPattern}><ClearIcon /></button>
      <button className={buttonClass} onClick={duplicatePattern}><DuplicateIcon /></button>
      <button className={buttonClass} onClick={removePattern}><DeleteIcon /></button>
    </div>
  );
}
...
function DuplicateIcon({}) {
  ...
  return (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-4 stroke-black">
...
function InvertIcon({}) {
  ...
  return (
    <svg fill="#000000" width="64px" height="64px" viewBox="-4 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 fill-black"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <title>invert</title> <path d="M0 6.656h24v18.688h-24v-18.688zM12 23.75v-2.219l5.531-5.531-5.531-5.531v-2.219h-10.406v15.5h10.406zM12 21.531l-5.531-5.531 5.531-5.531v11.063z"></path> </g></svg>
  )
}