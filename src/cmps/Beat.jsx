...
export default function Beat({ beat }) {
  const { intervalTime, isPlaying, currentLocation } = useSelector((state => state.player))
  const { patternIndex } = useContext(PatternContext)
  let counting = beat.division === 4 ? [beat.index + 1, "e", "+", "a"] : [beat.index + 1, "+", "a"];
  if (beat.division === 6) counting = [beat.index + 1, "a", "+", beat.index + 1, "a", "+"]
  ...
  const isCurrentBeat = currentLocation.atBeat === beat.index && currentLocation.atPattern === patternIndex
        ...
        {(provided, snapshot) => (
          ...
          >
            {counting.map((count, index) => {
              return (
                <ColItem noBorder={index === counting.length - 1}>
                  <BeatDivision
                    key={index}
                    divisionIndex={index}
                    count={count}
...
function DivisionPicker({beatIndex, division}) {
  ...
  return (
    <div className='relative w-full h-full' ref={dropdownRef}>
      ...
      > 
      {(ref) => (
        <div ref={ref} className='w-full flex flex-col absolute bottom-full bg-white border border-gray-300 p-2 shadow'>
        <button onClick={() => changeBeatDivision(beatIndex, 3)} className='py-1 w-full'>1/3</button>
        <button onClick={() => changeBeatDivision(beatIndex, 4)} className='py-1 w-full'>1/4</button>
        <button onClick={() => changeBeatDivision(beatIndex, 6)} className='py-1 w-full'>1/6</button>
