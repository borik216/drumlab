...
export default function PlayerControls({ play, stop, isPlaying, tempo }) {
    ...
    const buttonClass = "flex-1 border border-yellow-500"
    return (
        <div className="flex mx-auto max-w-3xl justify-between bg-slate-600 border rounded divide-x text-white w-full">
            <ControlsButton onClick={isPlaying ? stop : play}>
                <button>
                    {isPlaying ? <Stop w={'w-14'} h={'h-14'} /> : <Play w={'w-14'} h={'h-14'} />}
                </button>
            </ControlsButton>
            <ControlsButton>
                <label htmlFor="tempo" className={'text-md font-bold'}>BPM:
                    <input onChange={setTempo} type="number" id="tempo" className="lining-nums bg-slate-600 border-b text-center w-12 text-white border-b-indigo-50 ml-2" value={tempo} />
                </label>
            </ControlsButton>

            <ControlsButton>
                <DropMenu button={<DrumsIcon />}>
                    <InstrumentsPicker />
                </DropMenu>
            </ControlsButton>
            <ControlsButton onClick={toggleStrokes}>
                <button>
                    <Strokes />
                </button>
            </ControlsButton>
            <ControlsButton>
                <DropMenu button={<Repeat w={'w-14'} h={'h-14'}/>}>
                    <button onClick={() => dispatch({type: 'player/setRepeatAmount', payload: 'loop'})} className={'text-black'}>Loop</button>
                    <button onClick={() => dispatch({type: 'player/setRepeatAmount', payload: 1})} className={'text-black'}>1</button>
                    <button onClick={() => dispatch({type: 'player/setRepeatAmount', payload: 2})} className={'text-black'}>2</button>
                    <button onClick={() => dispatch({type: 'player/setRepeatAmount', payload: 3})} className={'text-black'}>3</button>
                    <button onClick={() => dispatch({type: 'player/setRepeatAmount', payload: 4})} className={'text-black'}>4</button>
                </DropMenu>
            </ControlsButton>
            <ControlsButton onClick={() => dispatch({type: 'player/toggleEditMode'})}>
                <EditIcon />
            </ControlsButton>
            
            <ControlsButton><SaveIcon /></ControlsButton>
        </div>
    )
}
